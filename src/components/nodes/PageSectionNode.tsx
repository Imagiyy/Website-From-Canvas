...
import { resolveNodeContent } from "../../utils/nodeResolver";
  ...
  const renderContent = () => {
    switch (node.type) {
      case "sectionHero": {
        return (
          <div data-node-id={node.id} style={{ width: "100%", height: "100%", background: "linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #334155 100%)", borderRadius: 12, padding: 32, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", gap: 16, boxSizing: "border-box", color: "#f1f5f9", fontFamily: "Inter, sans-serif", textAlign: "center", overflow: "hidden", position: "relative" }}>
            ...
            <div style={{ fontSize: Math.min(32, width * 0.06), fontWeight: 800, lineHeight: 1.1, zIndex: 1, letterSpacing: "-0.02em" }}>{content?.title || content?.text || "Build Something Amazing"}</div>
            <div style={{ fontSize: Math.min(16, width * 0.03), color: "#94a3b8", maxWidth: "80%", lineHeight: 1.5, zIndex: 1 }}>{content?.subtitle || "Create stunning websites with our intuitive drag-and-drop builder. No coding required."}</div>
            <div style={{ display: "flex", gap: 12, marginTop: 8, zIndex: 1 }}>
              <div style={{ padding: "10px 24px", background: "linear-gradient(135deg, #6366f1, #8b5cf6)", borderRadius: 8, fontSize: 13, fontWeight: 600, color: "#fff" }}>{content?.primaryButtonText || "Get Started"}</div>
              <div style={{ padding: "10px 24px", background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 8, fontSize: 13, fontWeight: 600, color: "#e2e8f0" }}>{content?.secondaryButtonText || "Learn More"}</div>
            </div>
          </div>
        );
      }
      ...
      case "sectionPricing": {
        ...
        return (
          <div data-node-id={node.id} style={{ width: "100%", height: "100%", background: "#0f172a", borderRadius: 12, padding: 24, display: "flex", flexDirection: "column", alignItems: "center", gap: 16, boxSizing: "border-box", color: "#f1f5f9", fontFamily: "Inter, sans-serif", overflow: "hidden" }}>
            <div style={{ fontSize: Math.min(22, width * 0.04), fontWeight: 700 }}>{content?.title || content?.text || "Choose Your Plan"}</div>
      ...
      case "sectionTestimonials": {
        ...
        return (
          <div data-node-id={node.id} style={{ width: "100%", height: "100%", background: "#0f172a", borderRadius: 12, padding: 24, display: "flex", flexDirection: "column", alignItems: "center", gap: 16, boxSizing: "border-box", color: "#f1f5f9", fontFamily: "Inter, sans-serif", overflow: "hidden" }}>
            <div style={{ fontSize: Math.min(22, width * 0.04), fontWeight: 700 }}>{content?.title || content?.text || "What Our Users Say"}</div>
      ...
      case "sectionTeam": {
        ...
        return (
          <div data-node-id={node.id} style={{ width: "100%", height: "100%", background: "#0f172a", borderRadius: 12, padding: 24, display: "flex", flexDirection: "column", alignItems: "center", gap: 16, boxSizing: "border-box", color: "#f1f5f9", fontFamily: "Inter, sans-serif", overflow: "hidden" }}>
            <div style={{ fontSize: Math.min(22, width * 0.04), fontWeight: 700 }}>{content?.title || content?.text || "Meet Our Team"}</div>
      ...
      case "sectionFeatures": {
        ...
        return (
          <div data-node-id={node.id} style={{ width: "100%", height: "100%", background: "#0f172a", borderRadius: 12, padding: 24, display: "flex", flexDirection: "column", alignItems: "center", gap: 16, boxSizing: "border-box", color: "#f1f5f9", fontFamily: "Inter, sans-serif", overflow: "hidden" }}>
            <div style={{ fontSize: Math.min(22, width * 0.04), fontWeight: 700 }}>{content?.title || content?.text || "Powerful Features"}</div>
      ...
      case "sectionCTA": {
        return (
          <div data-node-id={node.id} style={{ width: "100%", height: "100%", background: "linear-gradient(135deg, #4f46e5 0%, #7c3aed 50%, #a855f7 100%)", borderRadius: 12, padding: 32, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", gap: 16, boxSizing: "border-box", color: "#fff", fontFamily: "Inter, sans-serif", textAlign: "center", overflow: "hidden" }}>
            <div style={{ fontSize: Math.min(28, width * 0.05), fontWeight: 800, lineHeight: 1.1 }}>{content?.title || content?.text || "Ready to Get Started?"}</div>
            <div style={{ fontSize: Math.min(14, width * 0.025), color: "rgba(255,255,255,0.8)", maxWidth: "70%", lineHeight: 1.5 }}>{content?.subtitle || "Join thousands of creators building beautiful websites today. Start your free trial now."}</div>
            <div style={{ display: "flex", gap: 12, marginTop: 8 }}>
              <div style={{ padding: "12px 28px", background: "#fff", color: "#4f46e5", borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>{content?.primaryButtonText || "Start Free Trial"}</div>
              <div style={{ padding: "12px 28px", background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 8, fontSize: 14, fontWeight: 600, color: "#fff", cursor: "pointer" }}>{content?.secondaryButtonText || "Contact Sales"}</div>
            </div>
          </div>
        );
      }
        ...
        return (
          <div data-node-id={node.id} style={{ width: "100%", height: "100%", background: "#0f172a", borderRadius: 12, padding: 24, display: "flex", flexDirection: "column", gap: 16, boxSizing: "border-box", color: "#f1f5f9", fontFamily: "Inter, sans-serif", overflow: "hidden" }}>
            <div style={{ display: "flex", gap: 24, flex: 1 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 8 }}>{content?.brand || "CanvasSite"}</div>
                <div style={{ fontSize: 11, color: "#64748b", lineHeight: 1.5 }}>{content?.subtitle || "Build beautiful websites without writing code. Trusted by 10,000+ creators worldwide."}</div>
            ...
            <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 12, fontSize: 11, color: "#475569", textAlign: "center" }}>
              {content?.copyright || "© 2024 CanvasSite. All rights reserved."}
            </div>
          </div>
        );
      }
